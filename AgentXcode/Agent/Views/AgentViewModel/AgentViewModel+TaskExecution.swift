                    ...
                    if type == "text" {
                        ...
                        if name == "list_tools" {
                            ...
                            let builtIn = enabledTools.map { tool -> String in
                                // Extract actions from the "action" property description
                            ...
                            appendLog("🔧 Tools: \(enabledTools.count) built-in, \(mcpTools.count) MCP")
                            toolResults.append(["type": "tool_result", "tool_use_id": toolId, "content": output])
                        }
                        ...
                        if name == "xcode_analyze" {
                            let fp = input["file_path"] as? String ?? ""
                            appendLog("🔍 Analyzing: \(fp)")
                            flushLog()
                            let checker = SwiftCodeChecker()
                            let result: String
                            do {
                                let issues = try checker.runAllChecks(at: fp)
                                if issues.isEmpty {
                                    result = "No issues found in \(fp)"
                                } else {
                                    result = issues.map { "[\($0.severity)] Line \($0.line): \($0.description)" }.joined(separator: "\n")
                                }
                            } catch {
                                result = "Error analyzing \(fp): \(error.localizedDescription)"
                            }
                            appendLog(Self.preview(result, lines: 30))
                            commandsRun.append("xcode_analyze: \(fp)")
                            toolResults.append(["type": "tool_result", "tool_use_id": toolId, "content": result])
                        }

                        // Extract code snippet by line range (native)
                        if name == "xcode_snippet" {
                            let fp = input["file_path"] as? String ?? ""
                            let startLine = input["start_line"] as? Int
                            let endLine = input["end_line"] as? Int
                            appendLog("📋 Snippet: \(fp)")
                            flushLog()
                            let output: String
                            if let s = startLine, let e = endLine {
                                guard let data = FileManager.default.contents(atPath: fp),
                                      let content = String(data: data, encoding: .utf8) else {
                                    output = "Error: could not read \(fp)"
                                    toolResults.append(["type": "tool_result", "tool_use_id": toolId, "content": output])
                                    commandsRun.append("xcode_snippet: \(fp)")
                                    continue
                                }
                                ...
                                if start < end {
                                    ...
                                    let snippet = lines[start..<end].enumerated().map { (i, line) in
                                        "\(start + i + 1)\t\(line)"
                                    }.joined(separator: "\n")
                                    output = "```\(lang)\n\(snippet)\n```"
                                } else {
                                    output = "Error: invalid line range \(s)-\(e)"
                                }
                            } else {
                                output = (try? String(contentsOfFile: fp, encoding: .utf8)) ?? "Error: could not read \(fp)"
                            }
                        ...
                        // Code review — auto-discover Swift files, analyze each, produce summary
                        if name == "xcode_code_review" {
                            appendLog("🔍 Starting code review...")
                            flushLog()
                            ...
                            guard !swiftFiles.isEmpty else {
                                toolResults.append(["type": "tool_result", "tool_use_id": toolId, "content": "No Swift files found in \(reviewDir)"])
                                continue
                            }

                            // Create a plan
                            let steps = swiftFiles.map { "Analyze \(($0 as NSString).lastPathComponent)" }
                            let planInput: [String: Any] = ["action": "create", "name": "code-review", "title": "Code Review", "steps": steps.joined(separator: "\n")]
                            let _ = Self.handlePlanMode(action: "create", input: planInput, projectFolder: projectFolder, tabName: "main")
                            appendLog("📋 Plan created: \(swiftFiles.count) files to review")
                            flushLog()
                            ...
                            for (idx, file) in swiftFiles.enumerated() {
                                ...
                                appendLog("[\(idx + 1)/\(swiftFiles.count)] \(fileName)")
                                flushLog()

                                do {
                                    let checker = SwiftCodeChecker()
                                    let issues = try checker.runAllChecks(at: file)
                                    if !issues.isEmpty {
                                        let fileIssues = issues.map { "  [\($0.severity)] Line \($0.line): \($0.description)" }.joined(separator: "\n")
                                        allIssues.append("**\(fileName)** (\(issues.count) issues)\n\(fileIssues)")
                                    }
                                    // Update plan step
                                    let updateInput: [String: Any] = ["action": "update", "name": "code-review", "step": idx, "status": "completed"]
                                    let _ = Self.handlePlanMode(action: "update", input: updateInput, projectFolder: projectFolder, tabName: "main")
                                } catch {
                                    allIssues.append("**\(fileName)**: Error — \(error.localizedDescription)")
                                }
                            }

                            let summary: String
                            if allIssues.isEmpty {
                                summary = "# Code Review Complete\n\n✅ No issues found across \(swiftFiles.count) Swift files."
                            } else {
                                summary = "# Code Review Complete\n\n\(allIssues.count) file(s) with issues out of \(swiftFiles.count) reviewed:\n\n" + allIssues.joined(separator: "\n\n")
                            }
                            appendLog(summary)
                            commandsRun.append("xcode_code_review")
                            toolResults.append(["type": "tool_result", "tool_use_id": toolId, "content": summary])
                        }

                        // Plan mode
                        if name == "plan_mode" {
                            let action: String = input["action"] as? String ?? "read"
                            let output: String = Self.handlePlanMode(action: action, input: input, projectFolder: projectFolder, tabName: "main", userPrompt: prompt)
                            appendLog(output)
                            toolResults.append(["type": "tool_result", "tool_use_id": toolId, "content": output])
                        }
                        ...
                        // Fallback: route unhandled tools through executeNativeTool
