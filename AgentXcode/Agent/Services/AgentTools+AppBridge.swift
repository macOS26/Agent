...
extension AgentTools {
    ...
    private static let xcodeToolNames: Set<String> = ["xc", "xcode", "xcode_build", "xcode_run",
        "xcode_list_projects", "xcode_select_project", "xcode_add_file", "xcode_remove_file",
        "xcode_analyze", "xcode_snippet", "xcode_bump_version", "xcode_bump_build",
        "xcode_get_version", "xcode_grant_permission"]

    /// Combined filter: user prefs + group filter + Xcode gating.
    @MainActor private static func makeFilter(provider: APIProvider, activeGroups: Set<String>?, projectFolder: String) -> (String) -> Bool {
