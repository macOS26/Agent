...
/// Screenshot and visual verification utilities for Accessibility API.
/// Provides tools for capturing screenshots and verifying visual state.
enum AccessibilityScreenshot {
    ...
    static func captureScreenshot(x: CGFloat? = nil, y: CGFloat? = nil, width: CGFloat? = nil, height: CGFloat? = nil, windowID: Int? = nil) -> String {
        ...
        guard AccessibilityPermissions.hasAccessibilityPermission() else {
            return AccessibilityServiceHelpers.errorJSON("Accessibility/Screen Recording permission required.")
        }
        AccessibilityServiceHelpers.logAudit("captureScreenshot(x: \(x.map(String.init) ?? "nil"), y: \(y.map(String.init) ?? "nil"), w: \(width.map(String.init) ?? "nil"), h: \(height.map(String.init) ?? "nil"), windowID: \(windowID.map(String.init) ?? "nil"))")
        ...
        do {
            ...
            if process.terminationStatus != 0 {
                return AccessibilityServiceHelpers.errorJSON("screencapture failed with exit code \(process.terminationStatus)")
            }
            
            guard FileManager.default.fileExists(atPath: outputPath) else {
                return AccessibilityServiceHelpers.errorJSON("Screenshot file not created at \(outputPath)")
            }
            ...
            return AccessibilityServiceHelpers.errorJSON("Failed to capture screenshot: \(error.localizedDescription)")
        }
    }
    
    /// Capture a screenshot of all visible windows (requires Screen Recording permission)
    static func captureAllWindows() -> String {
        guard AccessibilityPermissions.hasAccessibilityPermission() else {
            return AccessibilityServiceHelpers.errorJSON("Accessibility/Screen Recording permission required.")
        }
        AccessibilityServiceHelpers.logAudit("captureAllWindows()")

        // Try to capture just the frontmost window instead of the entire screen
        let frontWindowID = AccessibilityElementFinding.frontmostWindowID()
        ...
        do {
            ...
            guard FileManager.default.fileExists(atPath: outputPath) else {
                return AccessibilityServiceHelpers.errorJSON("Screenshot file not created")
            }
